...
//leveraged the following code for the map (https://www.d3-graph-gallery.com/graph/choropleth_hover_effect.html)
  ...
  d3.queue()
    ...
    './us_export.csv', function(d) { data.set(d.code, +d.ExportDollar); })
    .await(ready);
...
function createbar() {
  ...
  function barc(data) {
    ...
    function renderbarc() {
      const yScale = scaleLinear().domain([0, myMax(data, 'ExportDollar')]).range([plotHeight2, 0]);
      const xScale = scaleBand().domain(all_countrynames).range([0, plotWidth2]);
      ...
      yAxis2.call(axisLeft(yScale).ticks(6).tickSize(3));
  ...
  //    console.log(data.filter(x => x.Year === year))
      ...
      svg2
        .selectAll('rect')
        .data(data) //.data(data.filter(x => x.Year === year))
        .join('rect')
...
function createline() {
  ...
  //  console.log(data)
    ...
    function renderlinec() {
      ...
      xAxis.call(axisBottom(xScale).tickFormat(d3.format("d")).tickSize(3))
      .selectAll('text')
      .attr("transform", function(d) {
        return "translate(" + this.getBBox().height*-1 + "," + this.getBBox().height + ")rotate(-45)";
      })
      yAxis.call(axisLeft(yScale).ticks(6).tickSize(3));
      xLabel
      .text('Year')
      .attr("font-size", "10px")
